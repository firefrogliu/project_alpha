...
%The proposed concept of fluid caching in flow channels has been inspired by how excess fluids are dealt with when performing some specific biochemical operations in flow-based microfluidic biochips today.  For example, \figname~\ref{fig:mixing} shows the snapshots when performing the mixing operation on a biochip. In \figname~\ref{fig:mixing}(a) and \ref{fig:mixing}(b), the two fluids that need to be mixed are loaded into the rotary mixer sequentially. Then, mixing operation is started by sealing the mixer and activating the three valves on the top side sequentially at a sufficiently high frequency ($\sim$100 Hz) (\figname~\ref{fig:mixing}(c)). It can be seen from \figname~\ref{fig:mixing}(a)--\ref{fig:mixing}(c) that excess fluids are kept in flow channels when performing the steps above. After completing the mixing operation, in \figname~\ref{fig:mixing}(d), the resulting mixture and excess fluids are removed from flow channels and sequentially transported to the target components or the waste port.
\begin{figure}[t]
    \centering
	 \subfloat[]{
